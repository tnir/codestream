...
function _getVersion(packageJsonFile: string): string | undefined {
	try {
		...
		throw new Error(`An error occurred trying to parse the version from '${packageJsonFile}`);
	}
}
...
function _updateAssemblyInfo(filename: string, version: string): void {
	try {
		...
		consoul.success(`Successfully updated '${filename}' with new version '${version}'`);
	} catch (err) {
		consoul.error(`An error occurred trying to save the version ${version} into '${filename}' -`);
		throw err;
	}
}
...
function _updateVsixManifest(filename: string, version: string): void {
	try {
		...
		const newFileContents = fileContents.replace(versionRegEx, `$1${version}$3`);
...
function _setVersion(packageJsonFile: string, version: string): void {
	try {
		const fileContents = fs.readFileSync(packageJsonFile, "utf-8");
		const versionRegEx = /("version":\s*)"([^"]+)",/g;
		const newFileContents = fileContents.replace(versionRegEx, `$1"${version}",`);
		fs.writeFileSync(packageJsonFile, newFileContents);
		consoul.success(`Successfully updated '${packageJsonFile}' with new version '${version}'`);
	} catch (err) {
		consoul.error(
...
function setVersionJetBrains(version: string, productRootPath: string): void {
	...
	try {
		...
		const newFileContents = fileContents.replace(versionRegEx, `version "${version}"$2`);
		fs.writeFileSync(versionFile, newFileContents);
		consoul.success(`Successfully updated '${versionFile}' with new version '${version}'`);
	} catch (err) {
		consoul.error(
...
function setVersionVisualStudio(version: string, productRootPath: string): void {
	try {
		_setVersion(`${productRootPath}/package.json`, version);
	} catch (err) {
		throw err;
	...
	try {
		_recursiveFindFiles(productRootPath, "AssemblyInfo.cs", version, _updateAssemblyInfo); // there are a few of these, and just in case another is added, find them recursively
	} catch (err) {
		throw err;
	}
	...
	try {
		_recursiveFindFiles(productRootPath, "SolutionInfo.cs", version, _updateSolutionInfo); // there is only one of these, but the recursive find makes it easier
	} catch (err) {
		throw err;
	}
}

export function setVersion(productRootPath: string, product: string, version: string): void {
	try {
