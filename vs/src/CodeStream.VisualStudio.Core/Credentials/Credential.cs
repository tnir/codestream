...
namespace CodeStream.VisualStudio.Core.Credentials
{
	public class Credential : IDisposable
	{
		...
		static Credential()
		{
			lock (LockObject)
			{
				UnmanagedCodePermission = new SecurityPermission(
		...
		public string Password
		{
			...
			{
				CheckNotDisposed();
				SecurePassword = SecureStringHelper.CreateSecureString(
		...
		public bool Delete()
		{
			...
			var target = string.IsNullOrEmpty(Target)
				? new StringBuilder()
		...
		public bool Load()
		{
			...
			using (
				NativeMethods.CriticalCredentialHandle credentialHandle =
					new NativeMethods.CriticalCredentialHandle(credPointer)
			)
			{
