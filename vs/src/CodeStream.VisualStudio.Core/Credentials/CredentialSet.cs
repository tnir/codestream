...
namespace CodeStream.VisualStudio.Core.Credentials
{
	public class CredentialSet : List<Credential>, IDisposable
	{
		...
		private void LoadInternal()
		{
			...
			{
				Trace.WriteLine(
					$"Win32Exception: {new Win32Exception(Marshal.GetLastWin32Error()).ToString()}"
				);
				return;
			}
			...
			List<NativeMethods.CriticalCredentialHandle> credentialHandles = ptrCredList
				.Select(ptrCred => new NativeMethods.CriticalCredentialHandle(ptrCred))
				.ToList();
