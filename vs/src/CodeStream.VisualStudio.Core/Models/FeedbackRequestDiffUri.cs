...
namespace CodeStream.VisualStudio.Core.Models
{
	public class FeedbackRequestDiffUri
	{
		private static readonly Regex Regex = new Regex(
			@"codestream-diff[:\\/\\/](?<reviewId>.+)[\\/](?<checkpoint>.+)[\\/](?<repoId>.+)[\\/](?<direction>.+)[\\/](?<filePath>.+)",
		...
		{
			Uri = new Uri(
				$"codestream-diff://{reviewId}/{(checkpoint.IsNullOrWhiteSpace() ? "undefined" : checkpoint)}/{repoId}/{direction}/{filePath}"
			);
			FileName = filePath;
		}
		...
		public static bool TryParse(string str, out FeedbackRequestDiffUri frDiffUri)
		{
			...
			frDiffUri = new FeedbackRequestDiffUri(
