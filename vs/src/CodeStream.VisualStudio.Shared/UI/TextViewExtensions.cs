...
namespace CodeStream.VisualStudio.Core.Extensions
{
	public static class TextViewExtensions
	{
		...
		public static bool TryDisposeProperty<T>(
		...
		public static bool TryDisposeListProperty(
			this PropertyCollection collection,
			string key,
			bool removeProperty = true
		)
			...
			return lines
				.Select(
					line =>
						new Range
						{
							Start = new Position(line.Start.GetContainingLine().LineNumber, 0),
							End = new Position(line.End.GetContainingLine().LineNumber, 0)
						}
				)
				.ToList();
		}
		...
		public static Placement GetGeometryPlacement(this IWpfTextView textView, SnapshotSpan span)
		{
			...
			if (g == null)
				return new Placement(0, viewportWidth);

			var width =
				g.Bounds.Width > viewportWidth || Math.Abs(g.Bounds.Width) < 0.0001
					? viewportWidth
					: g.Bounds.Width;
