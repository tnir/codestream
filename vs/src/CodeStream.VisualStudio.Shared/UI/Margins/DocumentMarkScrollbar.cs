	...
	internal class DocumentMarkScrollbar : FrameworkElement, ICodeStreamWpfTextViewMargin
	{
		...
		public ITextViewMargin GetTextViewMargin(string marginName)
		{
			return
				string.Compare(
					marginName,
					PredefinedCodestreamNames.DocumentMarkScrollbar,
		...
		public void OnTextViewLayoutChanged(object sender, TextViewLayoutChangedEventArgs e)
		{
			if (
				AnyTextChanges(
					e.OldViewState.EditSnapshot.Version,
					e.NewViewState.EditSnapshot.Version
				)
			)
			{
		...
		private void OnViewOrMarginVisibilityChanged(
			object sender,
		...
		{
			var needEvents = checkEvents && _textView.VisualElement.IsVisible && MarginActive;
		...
		protected override void OnRender(DrawingContext drawingContext)
		{
			...
			if (_search == null || Visibility != Visibility.Visible)
				return;
			...
			try
			{
				...
				for (var i = 0; i < markerCount; ++i)
				{
					...
					var y = Math.Floor(
						_verticalScrollBar.GetYCoordinateOfBufferPosition(
							match.TranslateTo(_textView.TextSnapshot, PointTrackingMode.Negative)
						)
					);
					if (y + MarkThickness > lastY)
					{
						...
						drawingContext.DrawRectangle(
							new SolidColorBrush(snapshotSpanMarker.Color.ConvertToMediaColor()),
							null,
							new Rect(x: rectX, y: rectY, width: rectWidth, height: rectHeight)
						);
					}
				}
			}
		...
		{
			private static readonly List<SnapshotSpanMarker> EmptyList =
				new List<SnapshotSpanMarker>(0);
			...
			public BackgroundMarkerPlacement(
			...
			)
			{
				...
				ThreadPool.QueueUserWorkItem(
					delegate
					{
						...
						{
							_markers = textView1.Properties.GetProperty<List<DocumentMarker>>(
						...
						foreach (var span in snapshot.Lines)
						{
							...
							var marker = _markers.FirstOrDefault(
								_ => _?.Range?.Start.Line == lineNumber
							);
							if (marker == null)
								continue;
							...
							newMatches.Add(
								new SnapshotSpanMarker(
									new SnapshotSpan(start, 1),
									ThemeManager.GetCodemarkColorSafe(marker?.Color)
								)
							);
						}
