...
namespace CodeStream.VisualStudio.Shared.UI.Margins
{
	public partial class DocumentMark : UserControl
	{
		...
		protected override void OnMouseEnter(MouseEventArgs e)
		{
			...
			ToolTip =
				$"{_viewModel.Marker.CreatorName}, {_viewModel.Marker.CreateAtDateTime.TimeAgo()} ({_viewModel.Marker.CreateAtDateTime.ToDisplayDate()}) "
			...
			try
			{
				...
				if (
					!(
						...
						is IToolWindowProvider toolWindowProvider
					)
				)
					return;
				...
				if (
					!(
						...
						is IComponentModel componentModel
					)
				)
				{
				...
				var activeEditor = componentModel
				...
				{
					_ = codeStreamService.ShowCodemarkAsync(
						_viewModel.Marker.Codemark.Id,
						activeEditor?.Uri.NormalizePath()
					);
		...
		private void Track(IComponentModel componentModel)
		{
			...
			_ = codeStreamAgentService?.TrackAsync(
				TelemetryEventNames.CodemarkClicked,
				new TelemetryProperties { { "Codemark Location", "Source File" } }
			);
		}
	}
}
