...
namespace CodeStream.VisualStudio.Shared.UI.SuggestedActions
{
	internal class CodemarkSuggestedActionsSource : ISuggestedActionsSource
	{
		private static readonly ILogger Log =
			LogManager.ForContext<CodemarkSuggestedActionsSource>();
...
#pragma warning restore 0067
		...
		)
		{
			...
			System.Diagnostics.Debug.WriteLine(
				$"HasSuggestedActions HasEditorSelection={!range.IsEmpty}"
			);
		...
		private IEnumerable<SuggestedActionSet> GetSuggestedActionsCore(IWpfTextView wpfTextView)
		{
			try
			{
				...
				{
					new CodemarkCommentSuggestedAction(
						_componentModel,
						wpfTextView,
				...
				{
					applicableActions.Add(
						new CodemarkIssueSuggestedAction(
							_componentModel,
							wpfTextView,
							_virtualTextDocument
						)
	...
	{
		public CodemarkCommentSuggestedAction(
			IComponentModel componentModel,
			IWpfTextView wpfTextView,
			IVirtualTextDocument textDocument
		)
			: base(componentModel, wpfTextView, textDocument) { }
	...
	{
		public CodemarkIssueSuggestedAction(
			IComponentModel componentModel,
			IWpfTextView wpfTextView,
			IVirtualTextDocument textDocument
		)
				...
				async delegate
				{
					...
					try
					{
						...
						if (sessionService.WebViewDidInitialize == true)
						{
							...
							_ = codeStreamService.NewCodemarkAsync(
								_virtualTextDocument.Uri,
								editorState?.Range,
								CodemarkType,
								"Lightbulb Menu",
								cancellationToken: cancellationToken
							...
							d = eventAggregator
								...
								.Subscribe(e =>
								{
									...
									catch (Exception ex)
									{
										Log.Error(
											ex,
											$"{nameof(CodemarkSuggestedActionBase)} event"
										);
									}
								});
						}
