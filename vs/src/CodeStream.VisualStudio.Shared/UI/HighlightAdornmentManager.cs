...
namespace CodeStream.VisualStudio.Core.Adornments
{
	...
	public class HighlightAdornmentManager : ICanHighlightRange, IDisposable
	{
		...
		public HighlightAdornmentManager(IWpfTextView textView)
		{
			...
			_highlightAdornmentLayer = textView.GetAdornmentLayer(
				PropertyNames.TextViewCreationListenerLayerName
			);
				...
				for (var i = range.Start.Line; i <= range.End.Line; i++)
				{
					...
					if (!_lineInfos.TryGetValue(i, out ITextViewLine lineInfo))
					{
						if (Log.IsVerboseEnabled())
						{
							try
							{
								var visible = _textView.TextViewLines
									.AsQueryable()
									.Select(
										_ =>
											_textView.TextSnapshot.GetLineNumberFromPosition(
												_.Extent.Start.Position
											)
									)
									.ToList();
								if (visible.Count() >= 1)
								{
									Log.Verbose(
										$"Could not find lineInfo for line={i}, only lines {visible.First()}-{visible.Last()} are visible"
									);
								}
							}
							catch (Exception ex)
					...
					if (lineStart == lineEnd)
					{
						...
						{
							placement = _textView.GetGeometryPlacement(
								new SnapshotSpan(
									lineInfo.Extent.Snapshot,
					...
					else
					{
						if (i == lineStart)
						{
							...
							var endLength =
								startPosition >= lineInfo.Extent.End.Position
									? 1
					...
					Canvas.SetLeft(
						element,
						range.Start.Character == 0 ? (int)_textView.ViewportLeft : placement.Left
					);
		...
		{
			var themedColor = VSColorTheme.GetThemedColor(
				EnvironmentColors.ScrollBarBackgroundColorKey
			);
			var brush = new SolidColorBrush(
				Color.FromRgb(themedColor.R, themedColor.G, themedColor.B)
			);
			if (brush.CanFreeze)
			{
				brush.Freeze();
		...
		private void CreateLineInfos(ITextView textView, IEnumerable<ITextViewLine> textViewLines)
		{
			...
			try
			{
				if (textViewLines == null || textView.IsClosed)
					return;

				foreach (var line in textViewLines)
				{
					// GetLineNumberFromPosition is 0-based
					var lineNumber = textView.TextSnapshot.GetLineNumberFromPosition(
