...
namespace CodeStream.VisualStudio.Shared.Services
{
	...
	public abstract class UserSettingsService
	{
		...
		public T TryGetValue<T>(string bucketName, string dataKey)
		{
			using (
				Log.CriticalOperation(
					$"{nameof(TryGetValue)} BucketName={bucketName} DataKey={dataKey}"
				)
			)
			{
		...
		{
			using (
				Log.CriticalOperation($"{nameof(Save)} BucketName={bucketName} DataKey={dataKey}")
			)
			{
				...
				try
				{
					...
					var loaded =
						Load(bucketName)
						?? new UserSettings { Data = new Dictionary<string, object>() };
		...
		private UserSettings Load(string bucketName)
		{
			using (Log.CriticalOperation($"{nameof(Load)} BucketName={bucketName}"))
			{
				...
				{
					return new UserSettings { Data = new Dictionary<string, object>() };
				}
				try
				{
					if (
						_settingsProvider.TryGetString(
							CollectionName,
							string.Format(PropertyFormat, bucketName),
							out var data
						)
					)
					{
