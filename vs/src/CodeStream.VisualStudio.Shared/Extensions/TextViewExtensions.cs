...
namespace CodeStream.VisualStudio.Shared.Extensions
{
	public static class TextViewExtensions
	{
		...
		/// <returns></returns>
		public static bool HasValidMarginRoles(this IWpfTextView wpfTextView) =>
			wpfTextView.Roles.HasValidMarginRoles();
		...
		/// <param name="wpfTextView"></param>
		/// <returns></returns>
		public static bool HasValidTaggerRoles(this IWpfTextView wpfTextView) =>
			wpfTextView.Roles.HasValidMarginRoles();
		...
		/// <param name="wpfTextView"></param>
		/// <returns></returns>
		public static bool HasValidDocumentRoles(this IWpfTextView wpfTextView) =>
			wpfTextView.Roles.HasValidDocumentRoles();
		...
		public static Tuple<ITextSnapshotLine, ITextSnapshotLine> GetLinesFromRange(
			this IWpfTextView wpfTextView,
			int start,
			int end
		)
		{
			...
			try
			{
				foreach (var line in wpfTextView.VisualSnapshot.Lines)
				{
					...
					var lineNumber = wpfTextView.TextSnapshot.GetLineNumberFromPosition(
						line.Extent.Start.Position
					);
					if (start == lineNumber)
					{
						startLine = line;
		...
		/// <returns></returns>
		public static Span? ToStartLineSpan(
			this IWpfTextView wpfTextView,
			int startLine,
		...
		)
		{
			...
			try
			{
				foreach (var line in wpfTextView.VisualSnapshot.Lines)
				{
					...
					if (
						startLine
						== wpfTextView.TextSnapshot.GetLineNumberFromPosition(
							line.Extent.Start.Position
						)
					)
					{
						sp = new SnapshotPoint(
							line.Snapshot,
							startPosition.HasValue
								? line.MinStartCharacter(startPosition.Value)
								: line.Start.Position
						);
						break;
					}
		...
		public static bool TryCreateSelectionRange(this IWpfTextView wpfTextView, out Range range)
		{
			if (!wpfTextView.Selection.IsEmpty)
			{
				...
				var lineStart = wpfTextView.TextSnapshot.GetLineFromPosition(
					wpfTextView.Selection.Start.Position
				);
				var characterStart =
					wpfTextView.Selection.Start.Position - lineStart.Extent.Start.Position;
				range.Start = new Position(lineStart.LineNumber, characterStart);

				var lineEnd = wpfTextView.TextSnapshot.GetLineFromPosition(
					wpfTextView.Selection.End.Position
				);
				var characterEnd =
					wpfTextView.Selection.End.Position - lineEnd.Extent.Start.Position;
		...
		/// <returns></returns>
		public static bool TryCreateCaretPosition(
			this IWpfTextView wpfTextView,
		...
		)
		{
			...
			{
				var line = wpfTextView.TextSnapshot.GetLineFromPosition(
		...
		public static EditorState GetEditorState(this IWpfTextView wpfTextView)
		{
			...
			System.Diagnostics.Debug.WriteLine(
				$"Range Start={selectionRange.Start.Line},{selectionRange.Start.Character} End={selectionRange.End.Line},{selectionRange.End.Character} Caret={caretPosition.Line},{caretPosition.Character} HasSelected={!wpfTextView.Selection.IsEmpty}"
			);
			...
			if (
				...
				&& selectionRange.End.Character == 0
