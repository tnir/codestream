...
it("renders default state 2", async () => {
	...
	MockedHostApi.instance = mockFoo;
	...
	await waitFor(() => {
		expect((container as any).textContent).toBe(
			"Open a Pull Request Choose two branches to start a new pull request.destination: mastersource: fooCancelCreate Pull Request "
		);
	});
});
